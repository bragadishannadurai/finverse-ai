...
const openai = new OpenAI({ apiKey: config.OPENAI_API_KEY || 'placeholder-key' });
...
export const analyzeReceiptWithAI = async (imageUrl: string): Promise<Record<string, unknown>> => {
  if (!config.OPENAI_API_KEY) return {};
...
const getLocalInvestmentAdvice = (ctx: FinancialContext): InvestmentAdviceCard[] => {
  const surplus = (ctx.totalIncome || 0) - (ctx.totalExpenses || 0);
  if (surplus <= 0) return [];

  const cards: InvestmentAdviceCard[] = [];
...
Format: [{"type":"...","name":"...","suggestedMonthlyAmount":number,"risk":"Low|Medium|High","expectedReturn":"...","rationale":"...","action":"...","icon":"emoji"}]`,
    ...
    logger.error('Investment advice AI error:', error);
    return getLocalInvestmentAdvice(context);
  }
};
